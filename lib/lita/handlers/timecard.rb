  ...
  module Handlers
    class Timecard < Handler
      ...
      def add_hours(response)
        add_time(response, response.matches[0][0].to_f)
      end

      def add_minutes(response)
        add_time(response, response.matches[0][0].to_f./(60.0).round(2))
      end

      def add_time(response, time_spent)
        activity = response.args[1]
        response.reply("logged! (#{time_spent} hours) work on (#{activity})")
      end
